function MappingBlock(mapDiv, timelineDiv) {
    ...
    var baseMaps = {
    'Callejero': L.tileLayer.provider('OpenStreetMap.Mapnik'),
    'Gris': L.tileLayer.provider('CartoDB.Positron'),
    'Satélite': L.tileLayer.provider('Esri.WorldImagery')
    };
    var noOverlayLayer = new L.GridLayer();
    var groupedOverlays = {'Superponer': {'Nada': noOverlayLayer}};
    ...
    $.each(mapData['wms'], function(index, data) {
        ...
        groupedOverlays['Superponer'][data.label] = wmsLayer;
    });
    L.control.groupedLayers(baseMaps, groupedOverlays, {
        exclusiveGroups: ['Superponer']
    }).addTo(map);
